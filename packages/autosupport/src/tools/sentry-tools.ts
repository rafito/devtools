...
export function createSentryTool(client: SentryClient): ToolBundle {
  ...
  const definitions: ToolDefinition[] = [
    {
      name: 'query_sentry',
      description:
        'Consulta a API do Sentry. Use issueId se o ticket veio do Sentry, ou query para buscar por palavras-chave.',
      input_schema: {
        type: 'object',
        properties: {
  ...
  async function execute(name: string, input: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      if (query) return await client.searchIssues(query)
      return { error: 'issueId ou query obrigatório' }
    } catch (err) {

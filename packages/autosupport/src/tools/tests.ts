...
export function createTestsTool(cfg: TestsToolConfig = {}): ToolBundle {
  ...
  const definitions: ToolDefinition[] = [{
    name: 'run_tests',
    description: 'Executa os testes unitários. Use após escrever o fix para verificar que não quebrou nada.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  async function execute(name: string, input: Record<string, unknown>): Promise<unknown> {
    ...
    try {
      ...
      const output = ((err.stdout ?? '') + (err.stderr ?? '') + err.message).slice(0, 6000)
